...
class Todo
{
    ...
    {
        $query = " SELECT todos.title, todos.id, todos.due_date, todos.status FROM todos INNER JOIN todo_app.users users ON todos.user_id = users.id WHERE users.telegram_id = :telegram_id";
        $stmt = $this->pdo->prepare($query);
        $stmt->execute([
